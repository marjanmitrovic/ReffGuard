...
export default function PrihlaskyPage() {
  ...
  const loadData = async (currentUserId: string) => {
    ...
    const matchIds = [...new Set(apps.map((item) => item.match_id))];
    ...
    const { data: matchesData, error: matchesError } = await supabase
      .from("matches")
      .select("id, match_date, match_time, competition, home_team, away_team, location")
      .in("id", matchIds);
  ...
  const myApplications = useMemo(() => {
    return applications.map((app) => {
      const match = matches.find((item) => item.id === app.match_id);
      return { app, match };
    });
    ...
    <main className="min-h-screen bg-slate-100 pb-24">
      ...
      <div className="mx-auto max-w-md px-4 py-4">
        ...
        <div className="space-y-4">
          {loading ? (
            ...
            myApplications.map(({ app, match }) => {
              if (!match) return null;
              return (
                <article key={app.id} className="rounded-3xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
                  <div className="mb-3 flex items-start justify-between gap-3">
                    ...
                    <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ring-1 ${getStatusClasses(app.status)}`}>{getStatusLabel(app.status)}</span>
                  </div>
                  <div className="space-y-2 text-sm text-slate-700">
                    ...
                    <div className="text-sm text-slate-500">Čas přihlášení: {formatDateTime(app.applied_at)}</div>
                  </div>
                  {app.status === "sent" ? (
                    <div className="mt-4">
                      <button type="button" onClick={() => handleCancel(app.id)} className="flex h-12 w-full items-center justify-center rounded-xl border border-rose-300 bg-white px-4 text-sm font-semibold text-rose-700 transition hover:bg-rose-50">
      ...
      <nav className="fixed bottom-0 left-0 right-0 border-t border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto grid max-w-md grid-cols-4 gap-2 px-3 py-3 text-center text-xs font-medium text-slate-600">
          <Link href="/zapasy" className="rounded-xl px-3 py-2 hover:bg-slate-100">Zápasy</Link>
          <Link href="/prihlasky" className="rounded-xl bg-slate-900 px-3 py-2 text-white">Přihlášky</Link>
