...
export default function AdminPage() {
  ...
  const summary = useMemo(() => {
    return {
      ...
      full: matches.filter((item) => item.status === "full").length,
  ...
  const filteredMatches = useMemo(() => {
    ...
    return matches.filter((item) => {
      ...
      const searchOk = !value || text.includes(value);
        ...
        <div className="mb-4 grid grid-cols-2 gap-2">
          {[
            ...
            <button
              key={item.value}
              type="button"
              onClick={() => setFilter(item.value as MatchFilter)}
        ...
        <div className="space-y-4">
          {loading ? (
            ...
            <div className="rounded-3xl bg-white p-6 text-center shadow-sm ring-1 ring-slate-200">
              ...
              <p className="mt-2 text-sm text-slate-500">Pro tento filtr nejsou žádné zápasy.</p>
            </div>
          ) : (
              ...
              return (
                <article key={match.id} className="rounded-3xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
                  <div className="mb-3 flex items-start justify-between gap-3">
                    ...
                    <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ring-1 ${getStatusClasses(match.status)}`}>
                  ...
                  <div className="space-y-2 text-sm text-slate-700">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span>
                        Přihlášeno: {matchCount.active} / Potřeba: {match.needed_refs}
                      </span>
                    </div>
                    <div className="text-sm text-slate-500">
                      Schváleno: {matchCount.approved} / {match.needed_refs}
                    </div>
                  </div>

                  <div className="mt-4 grid grid-cols-3 gap-2">
                    <Link href={`/admin/zapasy/${match.id}/prihlasky`} className="flex h-11 items-center justify-center rounded-xl bg-slate-900 px-3 text-xs font-semibold text-white transition hover:bg-slate-800">
                      Přihlášky
                    </Link>
                    <Link href={`/admin/zapasy/${match.id}`} className="flex h-11 items-center justify-center rounded-xl border border-slate-300 bg-white px-3 text-xs font-semibold text-slate-900 transition hover:bg-slate-50">
                      Upravit
                    </Link>
                    <button type="button" onClick={() => handleDelete(match.id)} className="flex h-11 items-center justify-center rounded-xl border border-rose-300 bg-white px-3 text-xs font-semibold text-rose-700 transition hover:bg-rose-50">
                      Smazat
                    </button>
                  </div>
                </article>
